...
"""
    ...
    def format_derivative_data(self, data: Dict, timeframe: str) -> str:
        """
        Format EMA derivative data (slopes, inflections, compression) for Claude

        Args:
            data: Timeframe data with derivatives
            timeframe: "5min" or "15min"

        Returns:
            Formatted string with derivative analysis
        """
        if 'derivatives' not in data or not data['derivatives']:
            return f"**{timeframe.upper()} DERIVATIVES**: No derivative data available yet"

        derivatives = data['derivatives']

        # Compression state
        compression = derivatives.get('compression', {})
        comp_state = compression.get('state', 'unknown')
        comp_value = compression.get('value', 0)
        comp_spread = compression.get('spread_pct', 0)

        # Inflection signals
        signals = derivatives.get('inflection_signals', {})
        signal_type = signals.get('type', 'none')
        signal_strength = signals.get('strength', 0)
        bullish_inf = signals.get('bullish_inflections', 0)
        bearish_inf = signals.get('bearish_inflections', 0)
        bullish_acc = signals.get('bullish_accelerations', 0)
        bearish_acc = signals.get('bearish_accelerations', 0)

        # Find EMAs with significant slopes or inflections
        fast_emas = [5, 10, 15, 20]
        significant_derivatives = []

        for ema_period in fast_emas:
            ema_key = f'MMA{ema_period}'
            if ema_key in derivatives:
                deriv = derivatives[ema_key]
                slope = deriv.get('slope', 0)
                slope_color = deriv.get('slope_color', 'gray')
                inflection = deriv.get('inflection_type', 'none')

                if inflection != 'none' or slope_color in ['dark_green', 'dark_red', 'light_green', 'light_red']:
                    slope_arrow = "↗️" if slope > 0 else "↘️" if slope < 0 else "→"
                    significant_derivatives.append(
                        f"  • MMA{ema_period}: {slope_arrow} {slope_color} slope | {inflection}"
                    )

        derivatives_str = "\n".join(significant_derivatives) if significant_derivatives else "  • No significant inflections detected"

        formatted = f"""
**{timeframe.upper()} EMA DERIVATIVES & MOMENTUM**:

📊 Compression: {comp_state.upper()} (value: {comp_value:.4f}, spread: {comp_spread:.2f}%)
   - highly_compressed = Range-bound, breakout imminent
   - expanding/highly_expanded = Trending, good for momentum

⚡ Inflection Signals: {signal_type.upper()} (strength: {signal_strength})
   - Bullish inflections: {bullish_inf} | Bearish inflections: {bearish_inf}
   - Bullish accelerations: {bullish_acc} | Bearish accelerations: {bearish_acc}

🔍 Fast EMA Slopes (Early Warning):
{derivatives_str}

💡 HOW TO USE DERIVATIVES:
- INFLECTIONS = Direction changes BEFORE color flips (10-30 sec early warning)
- BULLISH INFLECTION + COMPRESSED = Strong long setup forming
- BEARISH INFLECTION + COMPRESSED = Strong short setup forming
- ACCELERATIONS = Momentum building (2+ accelerations = strong signal)
- Dark/Light SLOPE COLORS = Rate of change intensity
"""
        return formatted
    ...
    def make_trading_decision(
        ...
        try:
            ...
            # Format derivative data (slopes, inflections, compression)
            derivatives_5min = self.format_derivative_data(data_5min, "5min")
            derivatives_15min = self.format_derivative_data(data_15min, "15min")
...
**STEP 1B: EMA DERIVATIVES - EARLY WARNING SYSTEM** 🎯

You now have access to EMA DERIVATIVES that show:
- **SLOPE** (1st derivative): Rate of change per EMA
  - Slope color: dark_green (strong up), light_green (weak up), gray (flat), light_red (weak down), dark_red (strong down)
  - Positive slope = EMA rising, Negative slope = EMA falling

- **ACCELERATION** (2nd derivative): Rate of change of slope
  - Positive acceleration = slope getting steeper (momentum building)
  - Negative acceleration = slope flattening (momentum dying)

- **INFLECTION POINTS**: Where EMAs change direction
  - bullish_inflection: Was falling, now rising (reversal signal)
  - bearish_inflection: Was rising, now falling (reversal signal)
  - bullish_acceleration: Rising and getting steeper (momentum building)
  - bearish_acceleration: Falling and getting steeper (momentum accelerating down)

- **COMPRESSION STATE**: How tight/wide the EMA ribbon is
  - highly_compressed: EMAs very close together, breakout imminent
  - compressed: Tight, good for range breakouts
  - normal: Average spread
  - expanding: Spreading apart, trending market
  - highly_expanded: Very wide, strong trend

**HOW TO USE DERIVATIVES FOR EARLY ENTRIES:**

1. **INFLECTION + COMPRESSION COMBO** (HIGHEST PRIORITY):
   ```
   LONG SETUP:
...
2. **ACCELERATION SIGNALS** (Momentum Confirmation):
   ```
...
3. **SLOPE COLOR PATTERNS**:
   ```
   - Fast EMAs (5, 10, 15) with dark_green slopes = Early bullish reversal
   - Fast EMAs with dark_red slopes = Early bearish reversal
   - Match slope colors with EMA colors for confirmation
   ```
...
4. **COMPRESSION + EXPANSION STRATEGY**:
   ...
   - This is the START of a big move
   ...
   → Good for momentum entries
   ```

**DERIVATIVE TIMING ADVANTAGE:**
- Derivatives detect momentum changes 10-30 seconds BEFORE color flips
- Traditional: Wait for colors to flip (late entry, miss 30-50% of move)
- Scalper: Enter on inflection signals (early entry, catch 80-90% of move)
- Backtest shows: 100% of ribbon flips had detectable inflection signals 10-17 seconds before!
