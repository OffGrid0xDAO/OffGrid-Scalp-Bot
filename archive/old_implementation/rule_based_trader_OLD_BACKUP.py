...
class RuleBasedTrader:
    ...
    def make_trading_decision(self, data_5min: dict, data_15min: dict, position=None, account=None, current_price: float = None, **kwargs):
        """
    ...
    def get_market_commentary(self, data_5min: dict, data_15min: dict):
        ...
        Returns insightful commentary based on current ribbon states
        """
        indicators_5min = data_5min.get('indicators', {})
        indicators_15min = data_15min.get('indicators', {})
        ...
        # Build insightful commentary
        commentary_parts = []

        # Trend analysis
        if state_5min == 'ALL_GREEN' and state_15min == 'ALL_GREEN':
            commentary_parts.append("🟢 STRONG BULLISH TREND - Both timeframes aligned green")
        elif state_5min == 'ALL_RED' and state_15min == 'ALL_RED':
            commentary_parts.append("🔴 STRONG BEARISH TREND - Both timeframes aligned red")
        elif state_5min == 'ALL_GREEN' and state_15min in ['MOSTLY_GREEN', 'ALL_GREEN']:
            commentary_parts.append("📈 BULLISH SETUP - 5min strong, 15min confirming")
        elif state_5min == 'ALL_RED' and state_15min in ['MOSTLY_RED', 'ALL_RED']:
            commentary_parts.append("📉 BEARISH SETUP - 5min strong, 15min confirming")
        elif 'MIXED' in state_5min or 'MIXED' in state_15min:
            commentary_parts.append("⚠️ CHOPPY CONDITIONS - Ribbon mixed, waiting for clarity")
        else:
            commentary_parts.append("⏸️ NEUTRAL - No clear trend")

        # EMA counts
        commentary_parts.append(f"| 5min: {pattern_5min['green_count']}G/{pattern_5min['red_count']}R")
        commentary_parts.append(f"| 15min: {pattern_15min['green_count']}G/{pattern_15min['red_count']}R")

        # Light EMAs (momentum)
        if pattern_5min.get('light_green_count', 0) >= 7:
            commentary_parts.append(f"| 💚 Strong momentum ({pattern_5min['light_green_count']} light green)")
        elif pattern_5min.get('light_red_count', 0) >= 7:
            commentary_parts.append(f"| ❤️ Strong momentum ({pattern_5min['light_red_count']} light red)")

        commentary = " ".join(commentary_parts)

        return {
            'commentary': commentary,
            'confidence': 0.75 if (state_5min == state_15min and 'ALL_' in state_5min) else 0.5
        }
