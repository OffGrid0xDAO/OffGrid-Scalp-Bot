#!/usr/bin/env python3
"""
Focused Backtest on USER's Trading Period ONLY
Period: Oct 5-21 (17 days where user made 22 trades)
Goal: Match user's +4.86% return
"""
...
print("\n" + "="*80)
print("🎯 FOCUSED BACKTEST: USER'S TRADING PERIOD")
print("="*80)
...
# Run bot backtest
print("\n" + "="*80)
print("🤖 RUNNING BOT BACKTEST (Iteration 5 params)")
print("="*80)

detector = EntryDetector(df_5m=df_5m_period, df_15m=df_15m_period)
df_signals = detector.scan_historical_signals(df_1h_period)
...
for i, trade in enumerate(trades, 1):
    ...
    print(f"\n{emoji} Trade #{i}: {trade['direction'].upper()}")
...
if capture_pct < 80:
    print("\n🎯 Target: 80%+ capture (close to user's performance)")
    print("\n📝 Recommendations:")
    print("   1. If bot has < 22 trades: LOOSEN filters")
    print("   2. If bot has > 22 trades: ADD quality filter")
    print("   3. If avg win < 2.36%: WIDEN take profit")
    print("   4. If avg loss < -0.56%: TIGHTER stop loss")
    print("   5. If win rate < 90%: IMPROVE entry timing")
