        ...
        for trade in self.backtest_trades:
            ...
            if time_diff.min() < pd.Timedelta(minutes=5):
                ...
                if trade['direction'].upper() == 'LONG':
                    ...
                    text = f"🔲 BACKTEST LONG<br>Entry: ${entry_price:.2f}<br>Confidence: {trade['confidence']:.0%}"
                else:
                    marker_color = '#FF1493'  # Deep pink
                    marker_symbol = 'square'
                    text = f"🔲 BACKTEST SHORT<br>Entry: ${entry_price:.2f}<br>Confidence: {trade['confidence']:.0%}"
            ...
            if time_diff.min() < pd.Timedelta(minutes=5):
                ...
                if trade['direction'].upper() == 'LONG':
                    ...
                    exit_symbol = 'diamond'
                    pnl_pct = trade.get('pnl_pct', 0)
                    text = f"💎 BACKTEST LONG EXIT<br>Exit: ${exit_price:.2f}<br>PnL: {pnl_pct:+.2f}%<br>{trade['exit_reason']}"
                else:
                    exit_color = '#FFB6C1'  # Light pink
                    exit_symbol = 'diamond'
                    pnl_pct = trade.get('pnl_pct', 0)
                    text = f"💎 BACKTEST SHORT EXIT<br>Exit: ${exit_price:.2f}<br>PnL: {pnl_pct:+.2f}%<br>{trade['exit_reason']}"
