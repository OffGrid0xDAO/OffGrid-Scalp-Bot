    ...
    def send_photo(self, photo_path: str, caption: str = "", parse_mode: str = 'HTML') -> bool:
        ...
        try:
            ...
            with open(photo_path, 'rb') as photo_file:
                ...
                data = {
                    'chat_id': self.chat_id,
                    'caption': caption[:1024] if caption else "",  # Telegram limit
                    'parse_mode': parse_mode
                }
...
"""
    ...
    def format_optimization_update(self,
                                   optimal_data: Dict,
                                   backtest_data: Dict,
                                   actual_data: Dict,
                                   recommendations: Dict,
                                   api_cost: float = 0.0) -> str:
        """
        Create comprehensive optimization cycle notification

        Shows 3-way comparison, missed opportunities, pattern findings, and planned improvements

        Args:
            optimal_data: Dict with optimal trades stats and patterns
            backtest_data: Dict with backtest trades stats and patterns
            actual_data: Dict with actual trades stats
            recommendations: Dict with Claude's recommendations
            api_cost: Cost of optimization API call
        ...
        opt_patterns = optimal_data.get('patterns', {})
        ...
        # Check both root level and patterns dict for backwards compatibility
        bt_patterns = backtest_data.get('patterns', backtest_data)
        ...
        # Calculate gaps
        missed_trades = opt_trades - bt_trades
        pnl_gap = opt_pnl - bt_pnl
        capture_rate = (bt_trades / opt_trades * 100) if opt_trades > 0 else 0

        # Execution gap
        execution_gap = bt_trades - act_trades

        # Pattern comparison
        opt_compression = opt_patterns.get('avg_compression', 0)
        bt_compression = bt_patterns.get('avg_compression', 0)
        opt_light_emas = opt_patterns.get('avg_light_emas', 0)
        bt_light_emas = bt_patterns.get('avg_light_emas', 0)

        # Recommendations
        key_findings = recommendations.get('key_findings', [])
        rule_changes = recommendations.get('rule_adjustments', {})

        # Build message
        message = f"""
🔧 <b>OPTIMIZATION CYCLE COMPLETE</b> 🔧

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

📊 <b>3-WAY PERFORMANCE COMPARISON</b>

🥇 <b>OPTIMAL TRADES</b> (Perfect Hindsight)
├ Trades: <code>{opt_trades}</code>
├ PnL: <code>{opt_pnl:+.2f}%</code>
├ Avg Hold: <code>{opt_avg_hold:.1f}min</code>
├ Avg Compression: <code>{opt_compression*100:.2f}%</code>
└ Avg Light EMAs: <code>{opt_light_emas:.0f}</code>

🥈 <b>BACKTEST TRADES</b> (Current Rules)
├ Trades: <code>{bt_trades}</code>
├ PnL: <code>{bt_pnl:+.2f}%</code>
├ Avg Hold: <code>{bt_avg_hold:.1f}min</code>
├ Win Rate: <code>{bt_win_rate*100:.1f}%</code>
├ Avg Compression: <code>{bt_compression*100:.2f}%</code>
└ Avg Light EMAs: <code>{bt_light_emas:.0f}</code>

🥉 <b>ACTUAL TRADES</b> (Live Execution)
├ Trades: <code>{act_trades}</code>
├ PnL: <code>{act_pnl:+.2f}%</code>
├ Avg Hold: <code>{act_avg_hold:.1f}min</code>
└ Win Rate: <code>{act_win_rate*100:.1f}%</code>

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

🎯 <b>GAP ANALYSIS</b>

📉 <b>Optimal → Backtest Gap</b>
├ Missed Trades: <code>{missed_trades}</code>
├ PnL Gap: <code>{pnl_gap:+.2f}%</code>
└ Capture Rate: <code>{capture_rate:.0f}%</code>

{'⚠️' if execution_gap > 0 else '✅'} <b>Backtest → Actual Gap</b>
├ Execution Diff: <code>{execution_gap:+d} trades</code>
{'└ Status: <b>Bot needs testing!</b>' if execution_gap > 0 else '└ Status: <b>Perfect alignment</b>'}
...
"""
        ...
        if key_findings:
            ...
            message += "No specific findings this cycle\n"

        message += "\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        message += "🛠️ <b>RULE IMPROVEMENTS PLANNED</b>\n\n"

        # Add rule changes
        if rule_changes:
            changes_count = 0
            for param, value in rule_changes.items():
                if changes_count < 8:  # Limit to 8 changes for readability
                    # Format parameter name
                    param_display = param.replace('_', ' ').title()
                    message += f"• {param_display}: <code>{value}</code>\n"
                    changes_count += 1

            if len(rule_changes) > 8:
                message += f"• ... and {len(rule_changes) - 8} more changes\n"
        else:
            message += "No rule changes recommended this cycle\n"

        message += "\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        message += f"💰 <b>API Cost:</b> <code>${api_cost:.4f}</code>\n"
        message += f"⏰ <code>{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</code>\n"
        message += "\n🔄 Next optimization in 30 minutes"

        return message.strip()

    def send_optimization_update(self,
                                 optimal_data: Dict,
                                 backtest_data: Dict,
                                 actual_data: Dict,
                                 recommendations: Dict,
                                 api_cost: float = 0.0) -> bool:
        """
        Send comprehensive optimization cycle notification
        ...
        Returns:
            bool: Success status
        """
        message = self.format_optimization_update(
