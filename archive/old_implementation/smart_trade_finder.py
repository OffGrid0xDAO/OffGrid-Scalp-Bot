...
class SmartTradeFinder:
    ...
    def find_smart_trades(self, hours_back: int = 24) -> Dict:
        ...
        for idx in range(len(df) - 1):
            ...
            # Try 'close' first, fallback to 'price'
            current_price = current.get('close', current.get('price', 0))
            # Try 'high' first, fallback to 'price'
            next_high = next_candle.get('high', next_candle.get('price', current_price))
            # Try 'low' first, fallback to 'price'
            next_low = next_candle.get('low', next_candle.get('price', current_price))
            ...
            # If in position, look for exit
                ...
                if exit_triggered:
                    ...
                    # Get entry candle data for additional metrics
                    entry_candle = df.iloc[entry_idx]

                    # Calculate compression at entry (if not in CSV, calculate it)
                    compression = entry_candle.get('ribbon_compression', None)
                    if compression is None or pd.isna(compression):
                        # Calculate compression from EMA values
                        ema_values = []
                        for ema in [5, 10, 15, 20, 25, 30, 40, 50, 60, 80, 100, 120]:
                            val_col = f'MMA{ema}_value'
                            if val_col in entry_candle and pd.notna(entry_candle[val_col]):
                                ema_values.append(float(entry_candle[val_col]))
                    ...
                    light_emas = 0
                    if entry_direction == 'LONG':
                        light_emas = self.count_light_emas(entry_candle, 'green')
                    else:
                        light_emas = self.count_light_emas(entry_candle, 'red')
        ...
        patterns = {
            'avg_compression': sum(t.get('compression', 0) for t in self.trades) / len(self.trades) if self.trades else 0,
            'avg_light_emas': sum(t.get('light_emas', 0) for t in self.trades) / len(self.trades) if self.trades else 0
        }
...
if __name__ == '__main__':
    ...
    if result['trades']:
        ...
        print(f"📊 Patterns - Avg Compression: {result['patterns']['avg_compression']:.4f}, Avg Light EMAs: {result['patterns']['avg_light_emas']:.1f}")
